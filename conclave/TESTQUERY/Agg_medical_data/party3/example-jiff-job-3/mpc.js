...
const stdDev = async function(inRel, keepRows, keyCol, aggCol, jiff_instance)
{
  ...
  exports.compute = function (input, jiff_instance) {
    ...
    var computation = Promise.all([dataShares, keepRowShares]).then(async function(arr) {
        ...
        var test_0_close = arr[0][1];
        var test_0_closeKeepRows = arr[1][1];

        var test_1_close = arr[0][2];
        var test_1_closeKeepRows = arr[1][2];

        var test_2_close = arr[0][3];
        var test_2_closeKeepRows = arr[1][3];

        var combined_dataRESULT = concatenate([test_0_close,test_1_close,test_2_close], [test_0_closeKeepRows,test_1_closeKeepRows,test_2_closeKeepRows]);
        var combined_data = combined_dataRESULT[0];
        var combined_dataKeepRows = combined_dataRESULT[1];

        var test_oblRESULT = await aggregate(combined_data, combined_dataKeepRows, 0, 1, jiff_instance);
        var test_obl = test_oblRESULT[0];
        var test_oblKeepRows = test_oblRESULT[1];

        var test_obl_open = open(test_obl, test_oblKeepRows);

        return test_obl_open;
