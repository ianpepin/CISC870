...
const stdDev = async function(inRel, keepRows, keyCol, aggCol, jiff_instance)
{
  ...
  exports.compute = function (input, jiff_instance) {
    ...
    var computation = Promise.all([dataShares, keepRowShares]).then(async function(arr) {
        ...
        var aggregation_0_close = arr[0][1];
        var aggregation_0_closeKeepRows = arr[1][1];

        var aggregation_1_close = arr[0][2];
        var aggregation_1_closeKeepRows = arr[1][2];

        var aggregation_2_close = arr[0][3];
        var aggregation_2_closeKeepRows = arr[1][3];

        var combined_dataRESULT = concatenate([aggregation_0_close,aggregation_1_close,aggregation_2_close], [aggregation_0_closeKeepRows,aggregation_1_closeKeepRows,aggregation_2_closeKeepRows]);
        var combined_data = combined_dataRESULT[0];
        var combined_dataKeepRows = combined_dataRESULT[1];

        var aggregation_oblRESULT = await aggregate(combined_data, combined_dataKeepRows, 0, 1, jiff_instance);
        var aggregation_obl = aggregation_oblRESULT[0];
        var aggregation_oblKeepRows = aggregation_oblRESULT[1];

        var aggregation_obl_open = open(aggregation_obl, aggregation_oblKeepRows);

        return aggregation_obl_open;
